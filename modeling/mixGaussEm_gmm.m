function [label, model, llh, R, break_flag] = mixGaussEm_gmm(X, init)
...
%     R = R(:,unique(label));   % remove empty clusters
    ...
    if abs(llh(iter)-llh(iter-1)) < tol*abs(llh(iter)); break_flag = true;break; end;
end
llh = llh(2:iter);
[~, resort_ind] = sort(model.mu, 'ascend'); % in case some components are with different orders during each node local computation
model.mu = model.mu(resort_ind);
model.Sigma = model.Sigma(1,1,resort_ind);
model.w = model.w(resort_ind);

back_label = zeros(size(label));
for iij = 1:size(R,2)
    back_label(label==iij) = find(resort_ind==iij);
end
label = back_label;
R = R(:,resort_ind);
